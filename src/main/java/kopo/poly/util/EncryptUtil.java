...
* 예측이 가능함에 따라서, 암호화할 때 암호화되는 값에 추가적인 문자열을 붙여서 함께 암호화 진행함
 */
    final static String addMessage = "polyDataAnalysis"; // 임의 값

    /*
    * AES128-CBC 암호화 알고리즘에 사용되는 초기 백터와 암호 키
     */

// 초기 백터(16비트를 가지고 있으며, 16바이트 단위로 암호화시, 암호화짤 총 길이가 16바이트가 되지 못하면 뒤에 추가되는 바이트)
...
public static String encHashSHA256(String str) {
    ...
    try {
        ...
        for (byte byteDatum : byteDate) {
            sb.append(Integer.toString(byteDatum & 0X100, 16).substring(1));
        }

        res = sb.toString();

                // 자바에서 제공하지 않는 알고리즘 아닌 경우, 에러 발생
 ...
 *  AES128 CBC 암호화 함수
 *  <p>
 *  128은 암호화 키 길이를 의미함 128 비트는 = 16바이트(1바이트=8비트 * 16 = 128)
 */
